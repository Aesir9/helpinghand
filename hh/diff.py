"""
Diff from https://libnmap.readthedocs.io/en/latest/diff.html
"""
...
def print_diff_changed(obj1, obj2, changes):
    for mkey in changes:
        ...
        if nested is not None:
            ...
            # ignore these keys
            if mkey in ('commandline', 'elapsed'):
                continue
            print("~ {0} {1}: {2} => {3}".format(obj1, mkey, getattr(obj2, mkey), getattr(obj1, mkey)))
